...
def getall():
    if request.method == 'POST':
        try:
            ...
            db.session.execute(query, {"id_usuario": session['user_id'], "sexo": sexo, "edad": fecha_nacimiento, "altura": altura, "peso": peso, "actividad": actividad, "calorias_quemadas": calorias_quemadas, "dia_semana": diasemana})
            db.session.commit()
